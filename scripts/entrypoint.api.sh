...
nr_enabled=$(cat ../.env | grep ENABLE_NR |awk -F= '{print $2}');
if [ "$nr_enabled" -eq "1" ]; then
  export NR_INSTALL_SILENT=true
  export NR_INSTALL_KEY=$(cat ../.env | grep NR_INSTALL_KEY | awk -F= '{print $2}')
  export nr_name=$(cat ../.env | grep NR_NAME | awk -F= '{print $2}')
