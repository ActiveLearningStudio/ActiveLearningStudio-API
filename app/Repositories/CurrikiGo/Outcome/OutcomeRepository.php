...
class OutcomeRepository implements OutcomeRepositoryInterface
{
    ...
    private function getStudentOutcomeData(array $data) {
        ...
        try {
                $activity_id = 46999;
                $submission_id = "Cg4Ilv2UiNIIENSf-vLBCg";
                $actor_id = 117479042634755818007;
                $check_submitted = \DB::select("select * from outcome_data where user_id = '117479042634755818007' AND submission_id = 'Cg4Ilv2UiNIIENSf-vLBCg' AND assignment_id = '46999' AND verb='submitted-curriki' order by page_order desc");
               if(count($check_submitted) > 0)
               {
                $data = \DB::select("select * from outcome_data where user_id = '117479042634755818007' AND submission_id = 'Cg4Ilv2UiNIIENSf-vLBCg' AND assignment_id = '46999' AND verb !='submitted-curriki' AND verb !='completed'  order by page_order");
                foreach($data as $result){
                    array_push($response,array(
                        ...
                        'children' => array(
                            ...
                            'answers' => array(
                                'score' => array(
                                    'raw' => intval($result->score_raw),
                                    'min' => intval($result->score_min),
                                    'max' => intval($result->score_max),
                                    'scaled' => intval($result->score_scaled),
                                ),
                                'response' => array($result->answer),
