...
class LibraryController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($entries as $library) {
            $usage = $interface->getLibraryUsage($library->id, $not_cached ? TRUE : FALSE);
            $settings['libraryList']['listData'][] = (object)array(
                'id' => $library->id,
        ...
        return view('h5p.library.index', compact('entries', 'settings', 'last_update', 'hubOn', 'required_files'));
    }
    ...
    private function assets($scripts = [], $styles = [])
    {
        ...
        $prefix = Storage::url('h5p/h5p-core/');
        $return = [
            'scripts' => [],
            'styles' => [],
        ];
    ...
    public function display_libraries_page()
    {
        ...
        print '<div class="wrap"><h2>' . esc_html__('Unknown task.') . '</h2></div>';
    }
...
//            JOIN h5p_libraries hl2 ON hl2.name = hl1.name
//            WHERE hl1.id = %d
...
//        // Do as many as we can in five seconds.
//        $start = microtime(TRUE);
//        $contents = $wpdb->get_results("SELECT id FROM h5p_contents WHERE filtered = ''");
//        $done = 0;
//        foreach ($contents as $content) {
//            $content = $core->loadContent($content->id);
//            $core->filterParameters($content);
...
//                    'upgrade',
//                    $id,
//                    $wpdb->get_var($wpdb->prepare('SELECT title FROM h5p_contents WHERE id = %d', $id)),
//                    $to_library->name,
