...
class H5PDragQuestionsUpdateSemanticsSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $h5pEssayLibParams = ['name' => "H5P.DragQuestion", "major_version" => 1, "minor_version" => 14];
        $h5pEssayLib = DB::table('h5p_libraries')->where($h5pEssayLibParams)->first();
        if ($h5pEssayLib) {
            DB::table('h5p_libraries')->where($h5pEssayLibParams)->update(['semantics' => $this->updatedSemantics()]);
        }
    }
